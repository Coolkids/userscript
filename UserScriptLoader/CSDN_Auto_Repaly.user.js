...
$(csdnHelperCss).html('\
.popWindow{position:fixed;z-index:10000;top:10px;left:10px;}\
.popWindow>span{display:block;text-align:left;color:cyan;text-shadow:0 0 2px white;background-color:#555;\
                box-shadow:-1px -1px 4px gray;margin:5px 0 0 0;padding:00 6px 0 6px;cursor:pointer;font-size: 13px;}\
');
...
$(".btn-comment").each(function(){
      ...
      var reg=/\/([_a-zA-Z0-9]+)\/([0-9]+)#/;
        ...
        //alert($(this).attr('href').match(reg));
...
//若待评论数大于0则提示用户滞留在本窗口
console.debug("本次任务总数--->"+GM_listValues().length);
if(GM_listValues().length>0){
  popWindow("待评论任务数："+GM_listValues().length,0);
...
for each(var stamp in GM_listValues()){
        // 发送评论
        //console.debug(stamp+":"+GM_getValue(stamp));
