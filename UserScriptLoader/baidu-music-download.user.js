...
// @run-at      document-end
// ==/UserScript==
var needcheck = false; //这个选项可以提醒你当超出容量时需要清理歌曲，因为百度云音乐本身提供了20000首歌的容量，如果你下载的歌曲超过这个数目就需要清理。我估计也没人下载到20000首歌，也就没有做过多测试。
...
var collect = function (callback) {
  var url = 'http://tingapi.ting.baidu.com/v1/restserver/ting?' + 'method=baidu.ting.favorite.addSongFavorite&format=json&from=bmpc&version=1.0.0&version_d=9.0.4.7&&baiduid=' + baiduid + '&songId=' + songid + '&time=' + (Math.round(new Date().getTime() / 1000));
  ajax(callback, url); //ajax回来的json待处理判断
};
var iscollect = function (callback) {
  var url = 'http://music.baidu.com/data/user/isCollect?type=song&ids=' + songid + '&r=' + Math.round(Math.random() * 100000) + (new Date().getTime());
  Eajax(function (data) {
    callback(JSON.parse(data).data.isCollect); //
  }, url);
};
var getsonglist = function (callback) {
  var url = 'http://tingapi.ting.baidu.com/v1/restserver/ting?method=baidu.ting.favorite.getCollectSong&format=json&from=bmpc&version=1.0.0&version_d=9.0.4.7&bdiduid=' + baiduid + '&pn=0&rn=50';
  ajax(function (data) {
...
var hasrate = function (rate, callback) {
  getsong(function (json) {
    if (rate == 999 && json.data.data.flac) {
      ...
      callback(json.data.data[rate].rate, json.data.data[rate].rate, true);
    } 
    else {
...
var download = function (rate, target) {
  hasrate(rate, function (truerate, format, ishas) {
    if (ishas) {
      var finalurl = 'http://yinyueyun.baidu.com/data/cloud/downloadsongfile?songIds=' + songid + '&rate=' + truerate + '&format=' + format;
...
var init = function () {
  ...
  script.innerHTML = '(' + initmenu.toString() + ')();';
